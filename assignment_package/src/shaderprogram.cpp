...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1), unifBindMatrices(-1),
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrIDs = context->glGetAttribLocation(prog, "vs_Joints");
    attrInf = context->glGetAttribLocation(prog, "vs_Influence");

    unifModel      = context->glGetUniformLocation(prog, "u_Model");
    unifModelInvTr = context->glGetUniformLocation(prog, "u_ModelInvTr");
    unifViewProj   = context->glGetUniformLocation(prog, "u_ViewProj");
    unifBindMatrices = context->glGetUniformLocation(prog, "u_BindMatrices");
    unifOverallTrans = context->glGetUniformLocation(prog, "u_OverallTransMatrices");
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrIDs != -1 && d.bindIDs()) {
        ...
        context->glVertexAttribIPointer(attrIDs, 2, GL_INT, 0, NULL);
    }

    if (attrInf != -1 && d.bindInf()) {
        context->glEnableVertexAttribArray(attrInf);
        context->glVertexAttribPointer(attrInf, 2, GL_FLOAT, false, 0, NULL);
    }
